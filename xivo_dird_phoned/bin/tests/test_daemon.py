...
class TestXivoDird(TestCase):
    ...
    def test_main_injects_argv_into_config_loading(self, load_config, controller_init, setup_logging,
                                                   change_user, pidfile_context):
        daemon.main(s.argv)

        load_config.assert_called_once_with(ANY, s.argv)

    def test_main_injects_config_in_controller(self, load_config, controller_init, setup_logging,
                                               change_user, pidfile_context):
    ...
    def test_main_when_config_user_then_change_user(self, load_config, controller_init, setup_logging,
                                                    change_user, pidfile_context):
        load_config.return_value = {
            'debug': s.debug,
            'log_filename': s.log_filename,
    ...
    def test_main_when_no_config_user_then_dont_change_user(self, load_config, controller_init, setup_logging,
                                                            change_user, pidfile_context):
        load_config.return_value = {
            'debug': s.debug,
            'log_filename': s.log_filename,
